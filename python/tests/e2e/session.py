    ...
    def test_session_roundtrip(self, resonite_session: Path) -> None:
        ...
        async def run() -> None:
            ...
            async with SessionClient() as client:
                ...
                # ResoniteLink を 0.6.0 publish 修正で全層コメントアウト。
                # client.apply_settings(resonite_link_enabled=...) と
                # snapshot の resonite_link_enabled / resonite_link_port が消えるため、
                # この e2e ブロックをコメントアウトする。GameLibs が World.ResoniteLink
                # API を公開したらコメントを外して復活させる。
                #
