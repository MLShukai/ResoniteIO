...
OS platform, Wine flag, engine/renderer host PIDs) over the Resonite IO
UDS and prints it. The snapshot is fixed at engine startup, so no
readiness retry is needed: the Info service answers as soon as the UDS
is bound. Assumes a Resonite client with the ResoniteIO mod loaded is
running on the host.
