...
- ✅ **proto lint**: `buf` (`buf.yaml`、`SERVICE_SUFFIX` + `RPC_REQUEST_STANDARD_NAME` + `RPC_RESPONSE_STANDARD_NAME` を except)。message 型は `CameraFrame` のようなモダリティ固有ドメイン名で命名する規約 (Step 3 で確立、根拠は `memory/agent/spec-driven-implementer/feedback_proto_rpc_naming_except.md`)
...
- [x] **proto** (`proto/resonite_io/v1/camera.proto`): `Camera.StreamFrames` (server-streaming) RPC。message 名はドメイン名 (`CameraFrame`, `CameraStreamRequest`) で命名し `RPC_REQUEST/RESPONSE_STANDARD_NAME` を except (規約: `memory/agent/spec-driven-implementer/feedback_proto_rpc_naming_except.md`)
- [x] **Core** (`ResoniteIO.Core.Camera`): `CameraService` 実装。`ICameraBridge` は optional DI で `null` なら `Status.Unavailable`、`CameraNotReadyException` は `FailedPrecondition` に翻訳。`fps_limit` で pacing (理論値 +2 のスラックで tolerance テスト、根拠: `memory/agent/spec-driven-implementer/feedback_streaming_fps_limit_test_tolerance.md`)。Fake Bridge による in-process streaming テスト済 (Resonite 不要)
- [x] **Mod** (`ResoniteIO.Bridge`): `FrooxEngineCameraBridge` でエージェント頭部 Slot に `Camera` コンポーネント生成 + `RenderTextureProvider` 読出。component graph 変更は `World.RunSynchronously` + `TaskCompletionSource` で engine tick 上にディスパッチ (パターン: `memory/agent/spec-driven-implementer/feedback_bridge_engine_thread_dispatch.md`)。BGRA8 でなく **RGBA8** で readback (commit `5129bb6` で青被り解消)
