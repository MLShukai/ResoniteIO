...
## 4. Sprint multiplier は ExternalInput 経路の自前再現
...
`sprint=true` のときに `Move.x/z *= sprint_multiplier` を自前で掛けて再現する。

proto field `sprint_multiplier` の意味:

- `0` (proto3 default) = 「server 既定値 `2.0` を使う」
- `>0` = override

**Why:** ExternalInput は binding 評価結果に加算されるため、multiplier を
自前で掛けないと sprint=false と挙動が変わらない。proto field で override
できるようにしたのは Python 側から走行速度を実機チューニングできるようにする
ため (server 側 const 変更を都度デプロイしなくていい)。

**How to apply:** e2e の sprint phase で「前進 phase より明らかに距離が
伸びている」ことを目視確認。差が体感より小さい場合は `Move` magnitude が
`_maxMagnitude` で normalize されている可能性があり、Python 側で
`LocomotionCmd(sprint=True, sprint_multiplier=3.0)` で再確認する。
