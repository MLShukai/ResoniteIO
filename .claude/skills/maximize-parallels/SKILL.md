...
- **同じ pytest セッション / port を奪い合う e2e**: `mod/tests/manual/` のように外部リソース (Resonite プロセス、UDS path、特定 port) を専有するテストや、`just deploy-mod` / `just resonite-start` のような単一 Resonite 実機を奪い合う操作は並列不可
