...
4. **自分のコードの内部関数モック → 禁止**: `SessionHost._BindAsync` / `CameraService._OnFrameInternal` のような内部メソッドを直接 `mocker.patch` / Moq で置き換える行為。リファクタで壊れるだけで何も保証しない
...
- `python/src/resoio/__init__.py` ↔ `python/tests/resoio/test_init.py`
- `python/src/resoio/cli/record.py` ↔ `python/tests/resoio/cli/test_record.py`
- `python/tests/` 直下に置くのは `__init__.py` / `helpers.py` / `conftest.py` / `fakes/` のみ
...
- `mod/src/ResoniteIO.Core/Session/SessionHost.cs` ↔ `mod/tests/ResoniteIO.Core.Tests/Session/SessionHostTests.cs`
- `mod/tests/ResoniteIO.Core.Tests/Common/` 配下に共通 fixture (`KestrelTestServer`, fake `I<Modality>Bridge` 実装等) を集約
- BepInEx 側 (`mod/src/ResoniteIO/`) は smoke 単位のみ `mod/tests/ResoniteIO.Tests/` で検証 (FrooxEngine 実機が必要な分は manual)

### Manual — `mod/tests/manual/`

- Resonite 実機を起動して end-to-end で振る舞いを確認するスクリプト群
- `dotnet run --project mod/tests/manual/<Scenario>.csproj` 形式で個別実行する想定
- CI 自動収集対象外。`just deploy-mod` で plugin を配置してから手動で動かす
...
| **manual / e2e**           | `mod/tests/manual/`                                                                      | end-to-end (実 Resonite、`just deploy-mod` 後の FrooxEngine + BepInEx + ResoniteIO loaded) | なし                                                                                   |
...
- 共有 fake の置き場所:
  - Python: `python/tests/fakes/<modality>.py`、テスト側は `from tests.fakes import FakeCameraBridge` で import
...
## manual シナリオ (`mod/tests/manual/`)

実 Resonite を起動して end-to-end で振る舞いを確認するシナリオ群。CI 自動収集対象外。`just deploy-mod` → `just resonite-start` → 手動操作 → `just log` で BepInEx LogOutput.log を tail して結果を判定する流れ。

- 各シナリオは目的 / 前提 / 手順 / 期待結果 / 撤収 を明示的に書いた `<Scenario>.md` または `<Scenario>.cs` (CLI driver) で残す
...
- 結果のスクリーンショット / ログを残す場合は `mod/tests/manual/_artifacts/<scenario>/<YYYYMMDD_HHMMSS>/` (gitignore 済み) に保存する想定
...
- 新マーカー候補: `api_contract` (公開 API 契約ピン)、`integration_real` (実 resource 必須の統合テスト)、`manual` (実 Resonite 必須、CI で自動 skip)
...
- `OnEngineReady` で SessionHost を bind し、`SafeShutdown` (BepInEx ProcessExit / 通常の Quit) で確実に Stop されるシーケンスは contract。Manipulation などの新規モダリティを追加する際にも、partial-failure を SafeShutdown が拾えるかを必ず確認する
