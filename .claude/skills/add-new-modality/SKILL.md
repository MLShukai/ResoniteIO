...
- **Mod 側** (`ResoniteIO.Tests`): FrooxEngine 依存があるため smoke test と Bridge adapter ロジック (engine API を呼ばない範囲) のみ。実 engine を要するシナリオは `python/tests/e2e/<modality>.py` を Claude が container 内で自動駆動する。Claude が自動化できない確認のみ `mod/tests/manual/` に markdown で残す
...
- `python/src/resoio/cli/` 配下に **action 名 flat command** で 1 ファイルずつ (`ping` / `record` / `mic` / `drive` / `grabber` / `display` 形式)
