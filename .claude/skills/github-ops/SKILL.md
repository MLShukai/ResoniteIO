...
resonite-io リポジトリでの GitHub 連携 (PR / issue / push) のための skill。**`gh` CLI と `git` はコンテナ内で実行する前提**で、Dockerfile に `gh` apt パッケージを同梱済み (`.claude/settings.container.json` で `Bash(git push:*)` / `Bash(gh:*)` を allow 済み)。

このリポジトリの git 運用規約 ([CLAUDE.md](../../../CLAUDE.md) の「Git 運用」「自走開発フロー」節) を `gh` 操作に落とし込む手順書。
...
- **`main` へのマージはユーザーが判断・実行する** ため、PR を立てたらレビュー待ち。Claude から `gh pr merge` は基本叩かない
...
- `gh pr merge` / `gh pr close` は **ユーザー判断**。Claude から自発的にマージ・close しない
...
`git push --no-verify` は使わない (CLAUDE.md と CI のガード趣旨に反する)。hook が落ちた根本原因 (`just run` の中で何が fail したか) を fix する。
