...
- **Mod 側** (`ResoniteIO.Tests`): FrooxEngine 依存があるため smoke test と Bridge adapter ロジック (engine API を呼ばない範囲) のみ。実 engine を要するシナリオは `python/tests/e2e/<modality>.py` を Codex が host-agent 経由で自動駆動する。Codex が自動化できない確認のみ `mod/tests/manual/` に markdown で残す
