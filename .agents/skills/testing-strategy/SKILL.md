...
- **Codex が container 内で `just resonite-launch` / `resonite-stop` で起動・停止し、screenshot は in-engine Camera (`CameraClient.shot()` = `resoio screenshot` CLI) で自動駆動する** のが基本路線。新規モダリティの実機検証は `python/tests/e2e/<modality>.py` を pytest harness として書く
- **新規モダリティでは e2e を必ず Codex が実装し、自分で実行して検証する (任意ではない)**。`just test` の green だけで「完了」としない。e2e green + 後述の screenshot 目視まで到達して初めて完了とする (`pgrep -af Renderite.Renderer.exe` で起動可否を事前確認する規約に従う)
...
| **e2e (Codex 自動)**       | `python/tests/e2e/`                                                                      | end-to-end (実 Resonite、`just deploy-mod` 後の FrooxEngine + BepInEx + ResoniteIO loaded、Codex が container 内 `just resonite-launch` で駆動) | なし                                                                                   |
...
実 Resonite を起動して end-to-end で振る舞いを確認するシナリオ群。**Codex が container 内で自動駆動するのが canonical**。`just deploy-mod` → `just resonite-launch` → pytest harness が gRPC client / Camera 録画 / `just log` 解析を回す → `just resonite-stop` の流れ。
...
- **可視的な状態変化を伴う操作 (ワールド移動・focus・UI 変化等) は、API 戻り値 assert に加えて操作前後で in-engine Camera の screenshot (`CameraClient.shot()` ベースの共有 `_screenshot()` ヘルパー、`resoio screenshot` CLI と同経路) を撮り、実際に変化したことを目視確認する** (例: `world.py` の join 前後 / focus 前後)。screenshot 検証まで含めて初めて e2e 完了とする
...
- 利用マーカー: `api_contract` (公開 API 契約ピン)、`e2e` (container 内 live Resonite 必須、`just e2e-test` 経由でのみ収集)。`integration_real` / `manual` のような横断マーカーは付けない (レイアウト分離だけで十分)
