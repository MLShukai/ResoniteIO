...
| Grabber | request/response | unary | [`GrabberClient`](../api/grabber.md) | Grab at the desktop cursor ray hit point / release, then operate the held / equipped item: `use` / `unuse` hold a button down with an analog `strength` (primary press pressure, e.g. BrushTool/Pen; left-click aligns / activates a tool, Pen-style press-drag-release), `equip` / `dequip` handle tools (desktop mode only). |
...
| Contact | request/response | unary | [`ContactClient`](../api/contact.md) | Manage the dash Contacts tab: list/search/add/accept/remove friends, with presence. |
| Auth | Python → Resonite | unary | [`AuthClient`](../api/auth.md) | Resonite cloud `login`/`logout`/`status` (gh-auth-like). |
