...
| Grabber | request/response | unary | [`GrabberClient`](../api/grabber.md) | Grab at the desktop cursor ray hit point / release, then operate the held / equipped item: `use` / `unuse` hold a button down (left-click aligns / activates a tool, Pen-style press-drag-release), `equip` / `dequip` handle tools (desktop mode only). |
