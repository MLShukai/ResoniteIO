...
- [Grabber engine 経路 (旧称 Manipulation)](feedback_grabber_engine_api.md) — Grab/Release は `InteractionHandler.Grabber.Grab(point,radius)`/`Release()` (掴むと HolderSlot へ reparent し手に自動追従、edge-triggered one-shot)。hand-pose は TrackedDevicePositioner が毎フレーム上書きし注入不可でスコープ外。ContextMenu と同形の unary RPC、home に grabbable 無く positive grab は manual。**Use/Unuse の hold は digital `Interact` + analog `Strength=1` の両方を毎 tick 注入しないと BrushTool/Pen が発火しない (tool は `primaryStrength` 駆動)。描画は `cursor set` 掃引で tip を動かす**。
