...
- [Manipulation Grabber 経路](feedback_manipulation_grabber.md) — Grab/Release は `InteractionHandler.Grabber.Grab(point,radius)`/`Release()` (掴むと HolderSlot へ reparent し手に自動追従、edge-triggered one-shot)。hand-pose は TrackedDevicePositioner が毎フレーム上書きし注入不可でスコープ外。ContextMenu と同形の unary RPC、home に grabbable 無く positive grab は manual。
- [Cursor は cursor lock で位置設定](feedback_cursor_lock_mechanism.md) — desktop カーソルは `InputBindingManager.RegisterCursorLock` で毎フレーム `WindowPosition` を強制 (OS injection `SetMousePosition` は Wine 不確実なので不採用)。正規化 \[0,1\] 座標、保持型。ContextMenu は engine-native 配置へ移行し中央表示は事前 `cursor.set_position(0.5,0.5)`。
...
- [code-quality-reviewer/MEMORY.md](agents/code-quality-reviewer/MEMORY.md) — レビュー観点 (dedup 閾値 / muxed checklist / pytest -k / skew evidence / Manipulation を HandleAsync に統合しない理由)
