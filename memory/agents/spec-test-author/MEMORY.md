...
- [Dash tabs/controls modality](project_dash_screens_modality.md) — wire-breaking rewrite from screens to tab-bar+controls; new proto/Core/Python surface, old Screen/Tree/Rect/Element removed
- [Resolver substring ambiguity tests](feedback_resolver_substring_ambiguity.md) — verify an "ambiguous" `_resolve_one` query is truly ambiguous (multiple items, not multiple keys) before committing; use `uv run python`
