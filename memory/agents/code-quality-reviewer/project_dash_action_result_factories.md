...
**How to apply:** 他モダリティの best-effort 操作 Bridge (Grabber 等) でも `<Modality>ActionResultSnapshot` 系を返すなら同じ NotFound/Rejected/Succeeded factory 三点セットを最初から置く。Service 層は既に generic `HandleAsync<TSnap,TProto>` + MapToProto overload で綺麗なので触らない。
