...
- **pixel 直指し操作(旧 Pointer 案)は不採用**。ref_id 直接操作で UI 操作が完結するため未実装。2026-06-13 の tab/control 刷新 (PR #37) で `DashRect` / `is_screen_space` は wire から完全撤去 (canvas-space rect は control の reading 順ソートに内部利用するのみ)。空間座標が要件化したら別途再設計する。
- **2026-06-13 刷新**: dash は **下部 tab バー (`RadiantDashScreen` = `DashTab`) + 現タブ内の interactable control (`Button`/`ScrollRect` = `DashControl`)** モデル。RPC は `ListTabs`/`SetTab`/`ListControls`/`Invoke`/`Scroll`/`Highlight`。旧 `GetTree`/`ListScreens`/`SetScreen` と `DashTree`/`DashElement`/`DashScreen` は撤去。control の label は `Button.LabelTextField` → 子 `Text.Content` → locale key → `Slot.Name` の順で解決 (空ラベル対策)。
