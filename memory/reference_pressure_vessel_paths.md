...
- **container の直接 umu-run 経路では `PRESSURE_VESSEL_FILESYSTEMS_RW` は効く**
  (2026-06-19 実機確認)。`resoio launch` (`python/src/resoio/launcher.py`) が `export PRESSURE_VESSEL_FILESYSTEMS_RW="$MOD_PATH"` してから `umu-run` を起動すると、
  `$MOD_PATH` (= `/workspace/gale`、`$HOME` 配下ではない) が sandbox に bind され、
...
(`just resonite-launch`)、mod (GrpcHost) は同 container 内 `/home/dev/.resonite-io/`
...
sandbox の既定 bind には含まれない。`resoio launch` (`python/src/resoio/launcher.py`) が mod 起動時に
`PRESSURE_VESSEL_FILESYSTEMS_RW="$MOD_PATH"` を export して sandbox に bind する
