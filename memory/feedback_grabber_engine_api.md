...
# Grabber modality の engine 経路と設計判断

> 2026-06-11 にモダリティ名を Manipulation から **Grabber** へ全層 rename (本文の型名は rename 後の現行名)。
...
\[\[feedback_microphone_engine_tap\]\]) と同様、本ファイルが Grabber 固有の engine 知見の一次正典。
...
`GrabberGrabRequest.point` (`WorldPoint`) は削除 (field 2 は reserved)。Grab は常に
...
- Cursor モダリティの永続保持 (\[\[feedback-cursor-lock-mechanism\]\]) と連動:
  `cursor.set_position` で照準 → `grab` (CLI: `resoio grabber`、旧称 `resoio grab` / `manipulate grab`) の流れが cross-RPC で成立する。
