...
- **コア層** (`ResoniteIO.Core` / `resoio`)
  ...
  - mod 側が注入する callback interface (`IConnectionBridge`, `ICameraBridge`, ...) を公開
...
**How to apply:** 新しいモダリティ Service を追加するときは必ず **(a) Core 側に Service + Bridge IF**、**(b) Mod 側に Bridge 実装** の 2 ペアを作る。proto の `<Protobuf GrpcServices="Server" />` は Core csproj に置き、mod csproj は Core を ProjectReference するだけ。BepInEx 依存・FrooxEngine 依存のコードが Core に紛れ込んでいたら設計違反として差し戻す。Python 側の新規 client (`CameraClient` 等) は `resoio.connection.ConnectionClient` と同じ async context manager パターンに揃える。
