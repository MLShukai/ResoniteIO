...
description: Locomotion Bridge は engine 既存の SmoothLocomotion / FirstPersonTargettingController / HeadSimulator に ExternalInput 経路で stateful repeater として注入する設計。look 用 cursor lock / pitch 符号 / AccessTools.FieldRefAccess の generic 引数順 / velocity 単位元 / consume-once jump / Reset + disconnect 検知 / Move body-local 変換 (LocalUserViewRotation) / Camera CameraStreamRequest 既存 bug 等の落とし穴を集約
metadata:
  type: feedback
---
...
Yaw / pitch は `ScreenCameraInputs.Look.Active` が
`InputInterface.IsCursorLocked` に gate されるため、Bridge が look 入力中だけ
低 priority の cursor lock を内部取得する。client は Cursor modality や手動 mouse
lock を事前に呼ぶ必要はない。lock の位置は現在の mouse window position で、既存
cursor lock がある場合は上書きしない。
...
- **look 用 cursor lock は Window focus までは作らない**:
  Bridge は yaw/pitch が非 0 の間だけ `RegisterCursorLock` で
  `ScreenCameraInputs.Look.Active` の前提を満たす。これは Resonite window が
  focused であることまでは代替しない (`IsCursorLocked = !unlock && IsWindowFocused`)。
  e2e は host-agent 起動の live Resonite で検証する。
