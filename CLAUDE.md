...
- **CLI** (`python/src/resoio/cli/`): action 名 flat command (`ping` / `wait` / `info` / `screenshot` / `record` / `mic` / `drive` / `grab` / `cursor` / `display` / `context-menu` / `dash` / `inventory` / `world` / `session` / `contact` / `auth` / `launch` / `terminate` / `shutdown`)。subgroup 階層化はしない。`record` は `--video` / `--audio` filter フラグ (両方未指定で muxed mp4/mkv) で Camera/Speaker を取得、`mic` は Microphone を流す、`drive` は対話 WASD、`grab` は action 省略時 grab、`screenshot` は in-engine Camera で PNG キャプチャ、`wait` は `Connection.Ping` が通るまで待つ readiness gate。設計規約は [cli-design skill](.claude/skills/cli-design/SKILL.md)
