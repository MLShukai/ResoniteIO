...
**現状: 当初計画の Step 0〜7 がすべて完了**。Step 0 = Docker 化開発環境、Step 1 = mod/python/proto スケルトン、Step 2 = `Session.Ping`、Step 3 = Camera server-streaming RPC、Step 4 = Locomotion client-streaming RPC、Step 5 = Speaker server-streaming RPC、Step 6 = Manipulation Grab/Release unary RPC、Step 7 = Microphone client-streaming RPC。加えて当初の 8-step 計画外の userspace / world 系モダリティ **Display / World / ContextMenu / Dash / Inventory / Cursor** (いずれも unary RPC 中心) も実装済み。Cursor は desktop カーソル位置を正規化座標で set/get するモダリティ (cursor lock で位置を保持)。ContextMenu は engine-native 配置に揃えてあり、`open` は現カーソル位置に開き視点移動で exit-lerp が自動クローズする (中央表示は事前に `cursor.set_position(0.5, 0.5)` でカーソルを中央へ寄せる)。
...
- C# Core (`mod/src/ResoniteIO.Core/`): モダリティ単位で IF と Service を `<Modality>/` 配下にまとめる (Session / Camera / Speaker / Microphone / Locomotion / Manipulation / Display / World / ContextMenu / Dash / Inventory / Cursor)。共通基盤として `UnixNanosClock` / `ILogSink` (`Logging/`)
...
- Python (`python/src/resoio/`): モダリティごとに `<Modality>Client` (`SessionClient` / `CameraClient` / `SpeakerClient` / `MicrophoneClient` / `LocomotionClient` / `ManipulationClient` / `DisplayClient` / `WorldClient` / `ContextMenuClient` / `DashClient` / `InventoryClient` / `CursorClient`) と `_socket.py` / `_generated/`
...
- proto: `proto/resonite_io/v1/{session,camera,locomotion,speaker,microphone,manipulation,display,world,context_menu,dash,inventory,cursor}.proto`
